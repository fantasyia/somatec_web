...
export function CatReceitaForm({ item }: { item?: RecipeCategory }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch('/api/admin/categorias-receitas', { method: item ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    const data = await res.json() as { ok: boolean; message?: string };
    if (data.ok) { router.push('/admin/categorias-receitas'); router.refresh(); }
    else { setError(data.message ?? 'Erro.'); setLoading(false); }
  }

  async function onDelete() {
    await fetch(`/api/admin/categorias-receitas?id=${item!.id}`, { method: 'DELETE' });
    router.push('/admin/categorias-receitas');
    router.refresh();
  }
